...
//constructor	
	...
	public void creatModel(String txt) {
		...
		String[][] arr = null;
		try {
			rs = studentManager.st.executeQuery("SELECT * FROM APP_STUDENT");
			ResultSetMetaData rsmd = rs.getMetaData();
			int cnt = 0;
			while(rs.next()) {	cnt++;	}
			rs.beforeFirst();
			
			arr = new String[cnt][rsmd.getColumnCount()];
			
			int row = 0;
			while(rs.next()) {
				for(int i = 1, j = 0; i <= rsmd.getColumnCount(); i++, j++) {
					arr[row][j] = rs.getString(i);
				}
				row++;
			}
			...
			case 1: // 번호를 입력받은 후  다른 4개의 항목을 수정할수 있도록함.
				...
				if(setNumDig()) {
					if(studentManager.isStudent(numDig)) {
						...
						Student studnet = studentManager.selectStudent(numDig);
						nameText.setText(studnet.getName());
						korText.setText(studnet.getKor()+"");
						engText.setText(studnet.getEng()+"");
						matText.setText(studnet.getMat()+"");
						step = 2;
